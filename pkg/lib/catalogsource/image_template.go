...
func UpdateKubeVersion(serverVersion *version.Info, logger *logrus.Logger) {
	...
	templateNameToReplacementValuesMap[TemplKubeMajorV] = strconv.FormatUint(semver.Major, 10)
	templateNameToReplacementValuesMap[TemplKubeMinorV] = strconv.FormatUint(semver.Minor, 10)
	templateNameToReplacementValuesMap[TemplKubePatchV] = strconv.FormatUint(semver.Patch, 10)
}
